...
contract ProductSales {
    ...
    event ProductSold(address indexed buyer, uint256 productId, uint256 quantity, uint256 amountPaid);

    event FundsTransferred(address indexed from, address indexed to, uint256 amount);

    function sellProduct(address _buyer, uint256 _productId, uint256 _quantity, uint256 _amountPaid) external {
